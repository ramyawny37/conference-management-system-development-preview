-- READ ONLY: forward-only 6.19.1 reconciliation verification.
-- Run only after a separately approved apply. Every boolean must be true.
...
-- Reconciliation identity, exact trigger/index inventory, preserved device and
-- Conference contracts, and observable row counts for the deployment record.
with expected_triggers(table_name,trigger_name,function_name) as (values
  ('device_security_credentials','device_security_credentials_lifecycle_guard','guard_device_security_credential_lifecycle'),
  ('user_device_authorizations','user_device_authorizations_security_credential_guard','guard_device_authorization_security_credential_state'),
  ('device_possession_challenges','device_possession_challenges_identity_guard','guard_device_possession_challenge_identity'),
  ('device_possession_challenge_consumers','device_possession_challenge_consumers_guard','guard_device_possession_challenge_consumer'),
  ('privileged_device_listing_sessions','privileged_device_listing_sessions_lifecycle_guard','guard_privileged_device_listing_session_lifecycle'),
  ('system_owner_credential_bootstrap_authorizations','system_owner_bootstrap_authorizations_lifecycle_guard','guard_system_owner_bootstrap_authorization_lifecycle'),
  ('system_owner_credential_recovery_authorizations','system_owner_recovery_authorizations_lifecycle_guard','guard_system_owner_recovery_authorization_lifecycle'),
  ('privileged_device_authorization_audit_log','privileged_device_authorization_audit_immutable','prevent_device_authorization_audit_mutation'),
  ('system_owner_device_authorization_operations','system_owner_device_authorization_operations_immutable','prevent_device_authorization_audit_mutation')
), actual_triggers as (
  select classes.relname table_name,triggers.tgname trigger_name,
    procedures.proname function_name
  ...
  ('device_security_credentials_active_primary_idx'),
  ('device_possession_challenges_expiry_idx'),
  ('system_owner_device_operations_target_created_idx')
), compatibility_functions(signature) as (values
...
select
  exists(select 1 from supabase_migrations.schema_migrations
    where name='webauthn_privileged_device_foundation_reconciliation_6_19_1')
    as reconciliation_history_present,
  (select count(*) from expected_triggers)=9
    and not exists(
      select * from expected_triggers
      except select table_name,trigger_name,function_name from actual_triggers)
    as exact_nine_reconciliation_triggers_present,
  not exists(select 1 from expected_indexes expected
    where to_regclass('public.'||expected.index_name) is null)
    as explicit_indexes_present,
  not exists(select 1 from compatibility_functions expected
    where to_regprocedure('public.'||expected.signature) is null)
    as onboarding_and_6_17_6_19_functions_present,
  exists(select 1 from pg_trigger where not tgisinternal
    ...
    as migration_6_17_triggers_present,
  exists(select 1 from pg_trigger where not tgisinternal
    and tgname='conferences_prevent_invalid_organization_change')
    as migration_6_18_trigger_present,
  (select count(*) from public.devices) as devices_row_count_for_audit,
  (select count(*) from public.user_device_authorizations)
    as user_device_authorizations_row_count_for_audit,
  (select count(*) from public.conference_members)
    as conference_members_row_count_for_audit,
  (select count(*) from public.conferences) as conferences_row_count_for_audit,
  (select count(*) from public.organization_members)
    as organization_members_row_count_for_audit;
