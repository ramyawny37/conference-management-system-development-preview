-- Resolve the recovery actor from the immutable server authorization, never
-- from a browser-local Conference/public device identity.
...
begin
  ...
  if p_actor_user_id is null then
    raise exception 'STABLE_DEVICE_RECOVERY_OWNER_REQUIRED' using errcode='22023';
  end if;
  ...
  credential := public.require_system_owner_webauthn_actor(
    recovery.owner_user_id,recovery.actor_public_device_id,recovery.credential_id
  );
  ...
  return pg_catalog.jsonb_build_object(
    'status','ready','credentialId',credential.id,
    'serverActorDeviceId',recovery.actor_public_device_id
  );
end;
$$;
