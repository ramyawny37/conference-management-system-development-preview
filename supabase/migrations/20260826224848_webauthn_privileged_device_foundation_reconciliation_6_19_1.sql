...
-- Forward-only reconciliation after 6.19. This migration deliberately has a
-- new identity; it does not replay or repair the history entry for 6.16.
-- Fail closed before persistent DDL if Development does not have the audited
-- lineage and absence state for which this reconciliation was prepared.
do $$
declare
...
begin
  ...
  if latest.name is distinct from
      'snapshot_sync_account_authorization_hardening_6_19_0'
    or latest.content_md5 is distinct from
      '3eaf058c2f2041587ed376aa93e3bc50' then
    raise exception 'EXPECTED_MIGRATION_6_19_0_IS_NOT_CURRENT';
  end if;

  if exists (
    select 1
      from supabase_migrations.schema_migrations migrations
     where migrations.name ilike '%6_16%'
        or migrations.name ilike '%webauthn%privileged%device%foundation%'
  ) then
    raise exception 'WEBAUTHN_FOUNDATION_MIGRATION_HISTORY_CONFLICT';
  end if;

  select format('%I.%I',namespaces.nspname,classes.relname)
    into conflicting_object
    from pg_class classes
    join pg_namespace namespaces on namespaces.oid=classes.relnamespace
   where classes.relname=any(array[
  ...
  if conflicting_object is not null then
    raise exception 'WEBAUTHN_FOUNDATION_RELATION_CONFLICT:%',conflicting_object;
  end if;

  select format('%I.%I',namespaces.nspname,procedures.proname)
    into conflicting_object
    from pg_proc procedures
    join pg_namespace namespaces on namespaces.oid=procedures.pronamespace
   where procedures.proname=any(array[
   ...
   limit 1;
  if conflicting_object is not null then
    raise exception 'WEBAUTHN_FOUNDATION_FUNCTION_CONFLICT:%',conflicting_object;
  end if;
  ...
  select triggers.tgname
   ...
   where not triggers.tgisinternal
     and triggers.tgname=any(array[
       'device_security_credentials_lifecycle_guard',
   ...
   limit 1;
  if conflicting_object is not null then
    raise exception 'WEBAUTHN_FOUNDATION_TRIGGER_CONFLICT:%',conflicting_object;
  end if;

  -- Known Supabase Auth WebAuthn internals are intentionally excluded. Any
  -- similarly purposed application relation outside auth requires review.
  select format('%I.%I',namespaces.nspname,classes.relname)
    into conflicting_object
    from pg_class classes
    join pg_namespace namespaces on namespaces.oid=classes.relnamespace
   where namespaces.nspname not in
     ...
     and (classes.relname ilike '%privileged%device%'
       or classes.relname ilike '%device%security%credential%'
       or classes.relname ilike '%possession%challenge%'
       or classes.relname ilike '%system%owner%device%authorization%')
   order by namespaces.nspname,classes.relname
   limit 1;
  if conflicting_object is not null then
    raise exception 'ALTERNATIVE_WEBAUTHN_FOUNDATION_REQUIRES_REVIEW:%',
      conflicting_object;
  end if;

  if to_regclass('public.user_device_authorizations') is null
    or to_regclass('public.devices') is null then
    raise exception 'DEVICE_AUTHORIZATION_FOUNDATION_MISSING';
  end if;
  if not exists (
    select 1
      from pg_constraint constraints
     where constraints.conrelid='public.user_device_authorizations'::regclass
       and constraints.contype='p'
       and (select array_agg(attributes.attname order by keys.position)
              from unnest(constraints.conkey)
                   with ordinality keys(attnum,position)
              join pg_attribute attributes
                on attributes.attrelid=constraints.conrelid
               and attributes.attnum=keys.attnum)
           =array['user_id','device_id']::name[]
  ) then
    raise exception 'USER_DEVICE_AUTHORIZATIONS_PRIMARY_KEY_INVALID';
  end if;

  select pg_get_userbyid(classes.relowner)
    into controlled_owner
    from pg_class classes
   where classes.oid='public.user_device_authorizations'::regclass;
  if controlled_owner is null then
    raise exception 'WEBAUTHN_RECONCILIATION_CONTROLLED_OWNER_MISSING';
  end if;

  if to_regprocedure('public.prevent_device_authorization_audit_mutation()') is null
    or to_regprocedure(
      'public.require_current_approved_device(uuid)') is null
    or to_regprocedure(
      'public.register_or_refresh_current_device(uuid,text,text)') is null
    or to_regprocedure(
      'public.request_current_device_authorization(uuid,uuid)') is null then
    raise exception 'DEVICE_ONBOARDING_CONTRACT_MISSING';
  end if;
  ...
  if exists (
    ...
    where to_regprocedure('public.'||signatures.signature) is null
  ) or (select count(*) from pg_trigger triggers
       where not triggers.tgisinternal and triggers.tgname in (
         'conference_members_require_organization_membership',
         'organization_members_protect_conference_memberships',
         'conferences_prevent_invalid_organization_change'))<>3 then
    raise exception 'CONFERENCE_HARDENING_CONTRACT_MISSING';
  end if;
end;
$$;

-- Stabilize audited application relations and retain an in-transaction
-- baseline. No application row is inserted, updated, deleted, or rewritten.
lock table public.devices,public.user_device_authorizations,
  public.conference_members,public.conferences,public.organization_members
  in share mode;

create temporary table migration_6_19_1_baseline (
...
insert into migration_6_19_1_baseline(object_kind,object_name,fingerprint)
select 'row_count','devices',count(*)::text from public.devices
union all select 'row_count','user_device_authorizations',count(*)::text
  from public.user_device_authorizations
union all select 'row_count','conference_members',count(*)::text
  from public.conference_members
union all select 'row_count','conferences',count(*)::text from public.conferences
union all select 'row_count','organization_members',count(*)::text
  from public.organization_members
union all
select 'function',signature,
  md5(pg_get_functiondef(to_regprocedure('public.'||signature)))
from unnest(array[
  'register_or_refresh_current_device(uuid,text,text)',
  'request_current_device_authorization(uuid,uuid)',
  'require_current_approved_device(uuid)',
  'require_conference_member_organization_membership()',
  'prevent_conference_member_organization_removal()',
  'prevent_invalid_conference_organization_change()',
  'device_guarded_apply_conference_snapshot(uuid,uuid,uuid,bigint,jsonb,text,text)',
...
union all
select 'trigger',triggers.tgname,md5(pg_get_triggerdef(triggers.oid,true))
from pg_trigger triggers
where not triggers.tgisinternal and triggers.tgname in (
  'conference_members_require_organization_membership',
  'organization_members_protect_conference_memberships',
  'conferences_prevent_invalid_organization_change');
...
-- Reconciliation-specific data and compatibility postconditions. The original
-- 6.16 structural/security postconditions above remain authoritative.
do $$
declare
  changed_object text;
begin
  if (select fingerprint from migration_6_19_1_baseline
       where object_kind='row_count' and object_name='devices')
       <> (select count(*)::text from public.devices)
    or (select fingerprint from migration_6_19_1_baseline
       where object_kind='row_count' and object_name='user_device_authorizations')
       <> (select count(*)::text from public.user_device_authorizations)
    or (select fingerprint from migration_6_19_1_baseline
       where object_kind='row_count' and object_name='conference_members')
       <> (select count(*)::text from public.conference_members)
    or (select fingerprint from migration_6_19_1_baseline
       where object_kind='row_count' and object_name='conferences')
       <> (select count(*)::text from public.conferences)
    or (select fingerprint from migration_6_19_1_baseline
       where object_kind='row_count' and object_name='organization_members')
       <> (select count(*)::text from public.organization_members) then
    raise exception 'WEBAUTHN_RECONCILIATION_APPLICATION_ROW_COUNT_CHANGED';
  end if;
  ...
  if not exists (
    ...
    raise exception 'USER_DEVICE_AUTHORIZATIONS_PRIMARY_KEY_CHANGED';
  end if;

  select baseline.object_name into changed_object
  from migration_6_19_1_baseline baseline
  where baseline.object_kind='function'
    and baseline.fingerprint is distinct from md5(pg_get_functiondef(
      to_regprocedure('public.'||baseline.object_name)))
  order by baseline.object_name limit 1;
  if changed_object is not null then
    raise exception 'EXISTING_FUNCTION_CHANGED:%',changed_object;
  end if;

  select baseline.object_name into changed_object
  from migration_6_19_1_baseline baseline
  join pg_trigger triggers on triggers.tgname=baseline.object_name
  where baseline.object_kind='trigger' and not triggers.tgisinternal
    and baseline.fingerprint<>md5(pg_get_triggerdef(triggers.oid,true))
  order by baseline.object_name limit 1;
  if changed_object is not null
    or (select count(*) from migration_6_19_1_baseline
         where object_kind='trigger')<>3
    or (select count(*) from pg_trigger triggers
         join migration_6_19_1_baseline baseline
           on baseline.object_kind='trigger'
          and baseline.object_name=triggers.tgname
        where not triggers.tgisinternal)<>3 then
    raise exception 'CONFERENCE_TRIGGER_CHANGED:%',changed_object;
  end if;
end;
$$;
