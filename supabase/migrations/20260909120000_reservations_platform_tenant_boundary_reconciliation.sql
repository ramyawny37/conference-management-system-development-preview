...
create or replace function reservations_private.resolve_platform_scope()
returns uuid
language plpgsql
stable
security definer
...
declare
  ...
  v_count integer;
begin
  select (array_agg(o.id order by o.id))[1], count(*)::integer
    into v_scope, v_count
  from public.organizations o
  where o.status = 'active'
    and o.is_default;

  if v_count <> 1 then
    raise exception 'RESERVATIONS_PLATFORM_SCOPE_UNAVAILABLE' using errcode = '42501';
  end if;
...
create or replace function reservations_private.context(p_device_id uuid, p_organization_id uuid, p_permission text)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  v_scope := reservations_private.resolve_platform_scope();

  if p_organization_id is distinct from v_scope then
    raise exception 'RESERVATIONS_SCOPE_OVERRIDE_DENIED' using errcode = '42501';
  end if;

  return v_context || jsonb_build_object('organizationId', v_scope);
end
$$;
...
begin
  if p_module <> 'reservations' then
    return platform.execute_device_operation_pre_reservations_event_booking(p_user_id, p_session_id, p_token_hash, p_module, p_operation, p_args);
  end if;
  ...
  if p_args is null or jsonb_typeof(p_args) <> 'object'
     or p_args ?| array['p_organization_id','organization_id','p_device_id','p_actor_device_id','p_actor_user_id'] then
  ...
  case p_operation
    when 'get_dashboard_summary' then perform platform_private.require_exact_jsonb_keys(p_args,array[]::text[]);
    when 'list_events' then perform platform_private.require_exact_jsonb_keys(p_args,array['p_status','p_limit']);
    ...
    when 'list_bookings' then perform platform_private.require_exact_jsonb_keys(p_args,array['p_event_id','p_limit']);
    when 'get_booking_detail','list_booking_payments','get_operational_state' then perform platform_private.require_exact_jsonb_keys(p_args,array['p_booking_id']);
    when 'search_participants_bookings' then perform platform_private.require_exact_jsonb_keys(p_args,array['p_query','p_event_id','p_limit']);
    when 'list_attendance','get_report_source_data' then perform platform_private.require_exact_jsonb_keys(p_args,array['p_event_id','p_limit']);
    when 'create_event' then perform platform_private.require_exact_jsonb_keys(p_args,array['p_operation_id','p_name','p_start_date','p_end_date','p_location','p_capacity','p_status','p_notes']);
    when 'update_event' then perform platform_private.require_exact_jsonb_keys(p_args,array['p_operation_id','p_event_id','p_expected_revision','p_name','p_start_date','p_end_date','p_location','p_capacity','p_status','p_notes']);
  ...
  v_args := p_args || jsonb_build_object('p_organization_id', reservations_private.resolve_platform_scope());
  if p_operation in('get_dashboard_summary','list_events','get_event','list_event_periods','list_booking_types','list_bookings','get_booking_detail','search_participants_bookings','list_booking_payments','list_attendance','get_operational_state','get_report_source_data') then
    return reservations.read(v_session.device_id,p_operation,v_args);
  end if;
  return reservations.mutate(v_session.device_id,p_operation,v_args);
end
$$;

revoke all on function reservations_private.resolve_platform_scope() from public, anon, authenticated, service_role;
revoke all on function reservations_private.context(uuid,uuid,text) from public, anon, authenticated, service_role;
grant execute on function reservations_private.resolve_platform_scope(), reservations_private.context(uuid,uuid,text) to postgres;
