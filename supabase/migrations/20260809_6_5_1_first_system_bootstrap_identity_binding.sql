...
alter table public.system_bootstrap_secret
  add column intended_user_id uuid null references auth.users(id) on delete cascade;

do $$
begin
  if exists(select 1 from public.system_bootstrap_secret) then
    raise exception 'BOOTSTRAP_SECRET_REPROVISION_REQUIRED';
  end if;
end;
$$;

alter table public.system_bootstrap_secret
  alter column intended_user_id set not null;
...
begin
  ...
  if exists(select 1 from public.system_user_roles where role='system_owner')
    or exists(select 1 from public.system_bootstrap_state where singleton_id=1 and completed_at is not null) then
    return jsonb_build_object('status','completed','setupRequired',false);
  end if;
  select intended_user_id into intended_id from public.system_bootstrap_secret where singleton_id=1;
  if not found then return jsonb_build_object('status','not_provisioned','setupRequired',false); end if;
  if intended_id<>actor_id then return jsonb_build_object('status','not_authorized','setupRequired',false); end if;
  return jsonb_build_object('status','setup_required','setupRequired',true);
end;
$$;
...
begin
  ...
  if crypt(coalesce(p_setup_token,''),secret_row.secret_hash)<>secret_row.secret_hash then raise exception 'BOOTSTRAP_CREDENTIAL_INVALID' using errcode='42501'; end if;
