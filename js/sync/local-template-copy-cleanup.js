(function(global){
  ...
  function cleanup(options){if(running)return running;running=preflight(options).then(function(checked){if(!checked.ok||checked.status==='already_clean')return checked;var d=dependencies(options),next=copy(d.data);next.houseTemplates=next.houseTemplates.filter(function(item){return String(item&&item.id||'')!==TARGET.templateId;});if(!next.houseTemplates.some(function(item){return String(item&&item.id||'')===TARGET.originalTemplateId;})||next.houseTemplates.length!==(d.data.houseTemplates||[]).length-1)throw new Error('TARGETED_REMOVAL_FAILED');if(!d.repository||typeof d.repository.saveAppSnapshot!=='function'||!d.storage||typeof d.storage.setItem!=='function')throw new Error('LOCAL_PERSISTENCE_UNAVAILABLE');return d.repository.saveAppSnapshot(next,{skipSyncQueue:true,skipTemplateSync:true}).then(function(){d.storage.setItem(global.SK,JSON.stringify(next));global.appData=next;d.data=next;return result(true,'cleanup_completed',{templateId:TARGET.templateId,cloudMutationPerformed:false});});}).catch(function(error){return result(false,'cleanup_failed',null,{code:String(error&&error.message||'LOCAL_CLEANUP_FAILED')});}).finally(function(){running=null;});return running;}
