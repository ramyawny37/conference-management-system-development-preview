(function(global){
  ...
  function mutate(input,foundation){
    ...
    var args={p_operation_id:id,p_action:input.action,p_target_user_id:target,p_module_key:MODULE,p_permission_key:String(input.permissionKey),p_grant_id:grantId,p_revocation_reason:input.action==='revoke'?String(input.revocationReason||'إلغاء الصلاحية من شاشة إدارة صلاحيات الموديولات').slice(0,300):null};
