...
function semanticExpression(name){
  const checks={
    ...
    '20260829130000':'to_regclass(\'public.module_permission_catalog\') is not null and to_regclass(\'public.module_permission_grants\') is not null',
    ...
    '20260905203000':"to_regprocedure('warehouse_private.canonicalize_unit_lines(jsonb)') is not null",
    ...
    '20260907140000':"to_regprocedure('public.require_effective_module_permission(uuid,text,text,text,text)') is not null",
...
function applyEntry(name){const file=migration(name),body=fs.readFileSync(path.join(root,file),'utf8'),executable=body.replace(/^\s*--[^\n]*$/gm,'').trim(),bounded=/^(begin|start\s+transaction)\s*;/i.test(executable)&&/commit\s*;\s*$/i.test(executable),semantic=semanticExpression(name);return {version:version(name),filename:file,sha256:sha(file),action:'APPLY',transaction:{startsExplicitly:/^(begin|start\s+transaction)\s*;/i.test(executable),commitsExplicitly:/commit\s*;\s*$/i.test(executable),safeOuterAtomicWrap:!bounded},preconditionSql:`select (not (${semantic}))::text`,verificationSql:`select (${semantic})::text`,prerequisites:['exact preceding controlled history tail','body-specific NOT_APPLIED detector'],expectedPostconditions:['semantic verifier independently succeeds','locked APPLY provenance recorded'],sourceReason:'Round 3L reviewed Production dependency order'};}
