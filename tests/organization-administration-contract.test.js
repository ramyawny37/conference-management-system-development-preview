...
async function run(){
  ...
  var guarded=runtime({rpc:function(name){if(name==='device_guarded_add_organization_member')return Promise.resolve({data:{status:'applied'},error:null});if(name==='device_guarded_get_my_organization_access')return Promise.resolve({data:access('organization_owner'),error:null});if(name==='device_guarded_list_organization_members')return Promise.resolve({data:[],error:null});}}),guardedResult=await guarded.service.addMember({organizationId:ids.organization,targetUserId:ids.target},Object.assign({},guarded.options,{deviceGuarded:true}));assert.strictEqual(guardedResult.ok,true);assert.strictEqual(guarded.calls[0].name,'device_guarded_add_organization_member');assert.strictEqual(guarded.calls[0].args.p_actor_device_id,ids.other);
