...
function scenario(authenticated,accountStatus,deviceStatus){
  ...
  const window={document:{getElementById:id=>nodes[id]},SupabaseAuth:{initialize:()=>Promise.resolve(),getState:()=>authState},SupabaseClientLayer:{getClient:()=>({auth:{onAuthStateChange:fn=>{listener=fn;return {data:{subscription:{}}};}}})},SystemAccessService:{initialize:()=>Promise.resolve(),refresh:()=>Promise.resolve(),getState:()=>({accountStatus:accountStatus,fresh:true})},CurrentDeviceAuthorizationUI:{initialize:()=>Promise.resolve(),refresh:()=>Promise.resolve(),getState:()=>({status:deviceStatus})},SyncSettingsUI:{}};
...
(async()=>{
  ...
  const noSession=await scenario(false,null,null);assert.strictEqual(noSession.result.status,'auth');assert(noSession.nodes.startupAccessGate.innerHTML.includes('sync_auth_email'));assert.strictEqual(noSession.home,0);assert.strictEqual(noSession.nodes.applicationBody.style.display,'none');
  ...
  approved.authState.authenticated=false;approved.listener('SIGNED_OUT',null);await new Promise(resolve=>setImmediate(resolve));assert.strictEqual(approved.nodes.applicationBody.style.display,'none');assert(approved.nodes.startupAccessGate.innerHTML.includes('sync_auth_email'));
