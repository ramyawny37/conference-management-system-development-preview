...
const cacheRevision='development-3-4-0-platform-device-rpc-cors-manifest-v1';
...
const userManagementStyleRevision='platform-shell-phase2b-v1';
...
assert.match(worker,/IS_DEVELOPMENT && requestUrl\.pathname === '\/manifest\.json'[\s\S]*fetch\(request\)\.catch\(\(\) => new Response\('\{\}'[\s\S]*status: 503/);
assert.match(worker,/platform-device-rpc-cors-manifest-v1/);
