...
(async function(){
  ...
  ['js/sync/current-device-authorization-ui.js?rev=platform-authorization-readiness-v1','js/sync/device-reauthorization-flow.js?rev=device-reauthorization-flow-v1','state.js?rev=develop-cross-store-arbitration-v1'].forEach(function(asset){assert.ok(index.includes(asset),asset);assert.ok(worker.includes('./'+asset),asset);});
