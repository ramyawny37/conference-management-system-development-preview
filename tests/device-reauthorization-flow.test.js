...
(async function(){
  ...
  ['js/sync/current-device-authorization-ui.js?rev=account-scoped-device-identity-v1','js/sync/device-reauthorization-flow.js?rev=device-reauthorization-flow-v1','state.js?rev=template-sync-isolation-v1'].forEach(function(asset){assert.ok(index.includes(asset),asset);assert.ok(worker.includes('./'+asset),asset);});
