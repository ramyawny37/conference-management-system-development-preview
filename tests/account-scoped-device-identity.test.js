...
(async()=>{
  ...
  const reconciled=sandbox.SupabaseDeviceIdentity.reconcileProvedIdentity({id:ids.memberDevice});assert.strictEqual(reconciled.success,true);assert.strictEqual(reconciled.status,'reconciled');assert.strictEqual(sandbox.SupabaseDeviceIdentity.getOrCreate().id,ids.memberDevice);assert.strictEqual(JSON.parse(values['dev:device-identity:'+ids.owner]).id,ids.memberDevice);
