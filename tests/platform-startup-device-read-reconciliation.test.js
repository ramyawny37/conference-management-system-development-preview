...
test("operation cardinalities and protected boundary remain unchanged",()=>{
  ...
  assert.equal(new Set(contract.EDGE_ONLY_PROTECTED.map(x=>x.operation)).size,60);
  assert.equal(new Set(contract.EDGE_ONLY_PROTECTED.map(x=>x.signature)).size,60);
